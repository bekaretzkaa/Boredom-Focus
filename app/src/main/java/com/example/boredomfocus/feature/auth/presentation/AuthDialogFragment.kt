...
import com.example.boredomfocus.feature.auth.presentation.AuthUiEvent
import com.example.boredomfocus.feature.auth.presentation.AuthUiState
import com.example.boredomfocus.feature.auth.presentation.AuthUiStatus
import com.example.boredomfocus.feature.auth.presentation.AuthViewModel
            ...
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.uiState.collect { state ->
                    ...
                    if(lastSignInMode != state.isSignIn) {
                        ...
                        animateButton(if(state.isSignIn) getString(R.string.auth_sign_in) else getString(
    ...
    private fun renderStatus(state: AuthUiState) {
        ...
        when(state.status) {
            ...
            is AuthUiStatus.Unknown, AuthUiStatus.GoogleFailed -> {
                ...
                binding.tvToast.text = if(state.status == AuthUiStatus.GoogleFailed) getString(R.string.auth_error_google) else getString(
